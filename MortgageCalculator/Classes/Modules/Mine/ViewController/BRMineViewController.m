...
- (UIView *)footerView {
    if (!_footerView) {
        ...
        btn.layer.cornerRadius = 20 * kScaleFit;
        btn.titleLabel.font = [UIFont systemFontOfSize:16.0f * kScaleFit];
        ...
        [btn mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.size.mas_equalTo(CGSizeMake(_footerView.width * 2 / 3, 40 * kScaleFit));
        }];
    }
    return _footerView;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        ...
        if (indexPath.row == 0) {
            ...
            // 切换语言
            BRLanguageViewController *languageVC = [[BRLanguageViewController alloc]init];
...
- (NSArray *)dataArr {
    if (!_dataArr) {
        _dataArr = @[
                     ...
                     @[@[@"mine_feedback.png", NSLocalizedString(@"意见反馈", nil)], @[@"mine_language.png", NSLocalizedString(@"切换语言", nil)], @[@"mine_cache.png", NSLocalizedString(@"清除缓存", nil)]], @[@[@"mine_about.png", NSLocalizedString(@"关于我们", nil)]]];
    }
    return _dataArr;
}
