...
- (void)setupFundLoanTotalPriceCell:(UITableViewCell *)cell {
    ...
    fundTotalPriceTF.placeholder = NSLocalizedString(@"单位：万元", nil);
...
- (void)setupBusinessLoanRatesCell:(UITableViewCell *)cell {
    ...
    businessLoanRatesTF.placeholder = NSLocalizedString(@"请输入商业贷款利率", nil);
...
- (void)setupFundLoanRatesCell:(UITableViewCell *)cell {
    ...
    fundLoanRatesTF.placeholder = NSLocalizedString(@"请输入公积金贷款利率", nil);
...
- (void)clickCalculatorBtn {
    ...
    if (self.businessTotalPriceTF.text.length == 0) {
        [MBProgressHUD showMessage:NSLocalizedString(@"请输入商业贷款金额", nil)];
        return;
    }
    if (self.fundTotalPriceTF.text.length == 0) {
        [MBProgressHUD showMessage:NSLocalizedString(@"请输入公积金贷款金额", nil)];
        return;
    }
    if (self.businessLoanRatesTF.text.length == 0) {
        [MBProgressHUD showMessage:NSLocalizedString(@"商业贷款利率不能为空", nil)];
        return;
    }
    if ([self.businessLoanRatesTF.text floatValue] < 1) {
        [MBProgressHUD showMessage:NSLocalizedString(@"商业贷款利率不能小于1%", nil)];
        return;
    }
    if (self.fundLoanRatesTF.text.length == 0) {
        [MBProgressHUD showMessage:NSLocalizedString(@"公积金贷款利率不能为空", nil)];
        return;
    }
    if ([self.fundLoanRatesTF.text floatValue] < 1) {
        [MBProgressHUD showMessage:NSLocalizedString(@"公积金贷款利率不能小于1%", nil)];
        return;
    }
    // 按房屋总价计算
...
- (NSArray *)tableDataArr {
    if (!_tableDataArr) {
        _tableDataArr = @[NSLocalizedString(@"商业贷款总额", nil), NSLocalizedString(@"公积金贷款总额", nil), NSLocalizedString(@"贷款期限", nil), NSLocalizedString(@"商业贷款利率", nil), NSLocalizedString(@"公积金贷款利率", nil), NSLocalizedString(@"还款方式", nil)];
    }
