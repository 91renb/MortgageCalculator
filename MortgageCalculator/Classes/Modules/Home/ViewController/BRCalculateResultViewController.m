...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title = NSLocalizedString(@"计算结果", nil);
    [self loadData];
    [self initUI];
}
...
- (UIView *)headView {
    if (!_headView) {
        ...
        titleLabel.text = NSLocalizedString(@"计算结果", nil);
        ...
        NSString *avgMonthRepayText = [NSString stringWithFormat:@"%@\n %@%@", NSLocalizedString(@"月均还款", nil), self.resultModel.avgMonthRepayment, NSLocalizedString(@"元", nil)];
        NSString *changeText = [NSString stringWithFormat:@"%@", self.resultModel.avgMonthRepayment];
        if (self.repayWay == BRRepayWayPriceSame) {
            avgMonthRepayText = [NSString stringWithFormat:@"%@\n %@%@", NSLocalizedString(@"首月还款", nil), self.resultModel.firstMonthRepayment, NSLocalizedString(@"元", nil)];
        ...
        if (self.calculateWay == BRCalculateWayUnitPriceAndArea) {
            avgMonthRepayText = [NSString stringWithFormat:@"%@\n %@%@", NSLocalizedString(@"首期付款", nil), self.resultModel.firstMonthRepayment, NSLocalizedString(@"元", nil)];
            changeText = [NSString stringWithFormat:@"%@", self.resultModel.firstMonthRepayment];
        }
        ...
        NSString *repayTotalPriceText = [NSString stringWithFormat:@"%@\n %@", NSLocalizedString(@"还款总额(元)", nil), self.resultModel.repayTotalPrice];
        ...
        NSString *loanTotalPriceText = [NSString stringWithFormat:@"%@\n %@", NSLocalizedString(@"贷款总额(元)", nil), self.resultModel.loanTotalPrice];
        ...
        NSString *repayTotalInterestText = [NSString stringWithFormat:@"%@\n %@", NSLocalizedString(@"累计利息(元)", nil), self.resultModel.repayTotalInterest];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    UILabel *numberLabel = [self getLabel:NSLocalizedString(@"期数", nil) centerX:VIEW_WIDTH / 8];
    [sectionHeaderView addSubview:numberLabel];
    UILabel *totalPriceLabel = [self getLabel:NSLocalizedString(@"月供(元)", nil) centerX:VIEW_WIDTH / 8 + VIEW_WIDTH / 4];
    [sectionHeaderView addSubview:totalPriceLabel];
    UILabel *priceLabel = [self getLabel:NSLocalizedString(@"本金(元)", nil) centerX:VIEW_WIDTH / 8 + VIEW_WIDTH * 2 / 4];
    [sectionHeaderView addSubview:priceLabel];
    UILabel *interestLabel = [self getLabel:NSLocalizedString(@"利息(元)", nil) centerX:VIEW_WIDTH / 8 + VIEW_WIDTH * 3 / 4];
