...
@interface BRFundLoanViewController ()<UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate>
{
    NSString *_calculateWayValue;  //计算方式
    NSString *_fundLoanTotalPriceValue;     //商业贷款总额
...
@property (nonatomic, strong) UITextField *fundLoanTotalPriceTF;    //公积金贷款总额
...
@implementation BRFundLoanViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    static NSString *const cellID = @"fundLoanCell";
...
- (void)setupFundLoanTotalPriceCell:(UITableViewCell *)cell {
    UITextField *fundLoanTotalPriceTF = [self getTextField:cell rightMargin:-36 * kScaleFit];
    ...
    fundLoanTotalPriceTF.placeholder = NSLocalizedString(@"单位：万元", nil);
        ...
        if (indexPath.section == 1 && indexPath.row == 1) {
            ...
            [BRStringPickerView showStringPickerWithTitle:NSLocalizedString(@"请选择贷款期限", nil) plistName:NSLocalizedString(@"loanTime", nil) defaultSelValue:self.loanTimeTF.text isAutoSelect:NO resultBlock:^(id selectValue) {
                ...
                if (loanYear <= 5) {
                    ...
                    self.loanRatesTF.text = _loanRatesValue = @"2.75";
                } else if (loanYear > 5) {
                    _currentStandRates = 3.25;
                    self.loanRatesTF.text = _loanRatesValue = @"3.25";
                }
            }];
        } else if (indexPath.section == 1 && indexPath.row == 3) {
            NSLog(@"选择还款方式");
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    ...
    if (textField.tag == 0) {
        ...
        _fundLoanTotalPriceValue = currentString;
