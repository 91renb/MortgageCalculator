...
#pragma mark - 获取文本的高度
+ (CGFloat)getTextHeight:(NSString *)text maxWidth:(CGFloat)maxWidth font:(UIFont *)font {
    if ([NSString isBlankString:text]) {
        return 0;
    }
    CGRect textRect = [text boundingRectWithSize:CGSizeMake(maxWidth, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:font} context:nil];
    // 向上取整，防止画UI时lab的高度取整
